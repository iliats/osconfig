...
// mockRunCommand replaces runCommand for tests and records calls.
func mockRunCommand(t *testing.T, responses map[string]cmdResponse) {
	t.Helper()
	original := runCommand
...
func TestDisableYumCronSystemd(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: errors.New("error stopping yum-cron: stop failed, out: "),
		},
		{
			desc: "disable fails",
			responses: map[string]cmdResponse{
			...
			wantErr: errors.New("error disabling yum-cron: disable failed, out: "),
		},
	}

	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			mockRunCommand(t, tt.responses)
...
func TestDisableYumCronChkconfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: errors.New("error checking status of yum-cron: chkconfig failed, out: "),
		},
		{
			desc: "disable fails",
			responses: map[string]cmdResponse{
			...
			wantErr: errors.New("error disabling yum-cron: off failed, out: "),
		},
	}
...
func TestDisableDnfAutomatic(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: errors.New("error checking status of dnf-automatic: list failed, out: "),
		},
		{
			desc: "stop fails",
			responses: map[string]cmdResponse{
			...
			wantErr: errors.New("error stopping dnf-automatic: stop failed, out: "),
		},
		{
			desc: "disable fails",
			responses: map[string]cmdResponse{
			...
			wantErr: errors.New("error disabling dnf-automatic: disable failed, out: "),
		},
	}
