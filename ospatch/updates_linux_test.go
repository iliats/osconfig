...
func mockRPM(t *testing.T, ctx context.Context, exists bool) {
	...
	// statFile, err := os.CreateTemp("", "proc_stat")
	// if err != nil {
	// 	t.Fatalf("Failed to create temp file for proc/stat mock: %v", err)
	// }
	// t.Cleanup(func() { os.Remove(statFile.Name()) })
	// fmt.Fprintf(statFile, "btime 2000\n")
	// statFile.Close()
	// procStatPath = statFile.Name()
}
